...
(function () {
  ...
  st.textContent =
    ...
    "#cdo-sim input[type=range]{accent-color:#d97757;width:150px}" +
    "#cdo-sim .val{color:#e8916f}" +
  ...
  function draw() {
    ...
    for (var p = 0, i = 0; p < HOURS; p += s.period, i++) {
      el("rect", { x: x(p), y: 18, width: x(Math.min(p + s.period, HOURS)) - x(p), height: H - 52,
        fill: i % 2 ? "rgba(217,119,87,0.05)" : "rgba(217,119,87,0.11)" });
    ...
    s.pulls.forEach(function (pl, k) {
      ...
      el("rect", { x: px - 4, y: H - 34 - hgt, width: 8, height: hgt, rx: 2,
        fill: pl.ok ? "#d97757" : "#7f1d1d", stroke: pl.ok ? "none" : "#ef4444" });
      el("text", { x: px - 4, y: H - 40 - hgt, "font-size": 10,
        fill: pl.ok ? "#e8916f" : "#ef4444", "font-family": "JetBrains Mono,monospace" },
        pl.ok ? "✓" : "✗");
      lastTxt = pl.ok
